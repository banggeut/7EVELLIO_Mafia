...
4. Environment Variables에 `.env.example`의 항목을 모두 등록
   ...
   - `NODE_ENV=production` 도 추가해주세요
   - **`JWT_SECRET` 필수**: 아무도 모르는 긴 문자열을 넣어주세요. 없으면 서버가 시작되지 않습니다.
     (없는 상태로 켜면 누구나 관리자 권한을 위조할 수 있어서 일부러 막아두었습니다)
     만드는 법: `node -e "console.log(require('crypto').randomBytes(48).toString('hex'))"`
     이 값을 나중에 바꾸면 모든 사람이 한 번씩 다시 로그인해야 합니다.
   - `HONOR_DATA_PATH` / `ACHIEVEMENT_DATA_PATH`: Persistent Disk를 붙였다면 그 경로 아래로
     지정해주세요. 지정하지 않으면 재배포할 때마다 명예·전적·업적 기록이 사라집니다.
...
- **채팅은 게임 화면 안에서만**: 치지직 채팅 중계는 쓰지 않습니다. 낮 토론·변론·비밀 채팅
  모두 게임 화면의 입력창으로 주고받습니다. (`server/src/chzzkChat.js`는 더 이상 쓰이지 않습니다)
- **재접속 처리**: 플레이어가 새로고침하거나 연결이 끊겼다가 다시 붙는 경우에 대한
  처리가 아직 단순합니다(세션 쿠키로 같은 channelId면 자동으로 다시 이어집니다만,
  중간에 놓친 상태 갱신에 대한 별도 예외처리는 없습니다).
- **입력 검증/부정행위 방지**: 자격 검증(내 직업이 아니면 능력 사용 불가 등)과 값 검증,
  소켓 요청 속도 제한(채팅 초당 2회, 그 외 초당 8회)이 서버에 들어가 있습니다.
  다만 방송 화면(`?mode=broadcast`) 소켓은 여전히 누구나 붙을 수 있습니다.

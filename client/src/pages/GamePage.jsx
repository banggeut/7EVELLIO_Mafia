...
function NightView({ theme, state, socket }) {
  ...
  const hasMedium = true; // 서버가 채널 접근 여부를 이미 걸러주므로 여기선 항상 채팅 UI만 노출
  ...
  return (
    <Card theme={theme}>
      ...
      {!state.myAbility && state.myAlive && state.myRole !== "lover" && state.myRole !== "medium" && (
  ...
  return (
    <Card theme={theme}>
      <div style={{ textAlign: "center", padding: "10px 0" }}>
        <div style={{ fontSize: 44 }}>{state.winner === "mafia" ? "🗡️" : "🌾"}</div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 4, textAlign: "left", marginBottom: 20 }}>
          {state.players.map((p) => <PlayerRow key={p.id} theme={theme} player={{ ...p, alive: true }} sub={p.roleLabel + (p.alive ? "" : " · 사망")} />)}
        </div>
